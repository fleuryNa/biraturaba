...
class Particularite extends BaseController
{
 ...
 protected $db;
    ...
    {
        $data['title'] = 'Liste des Particularites';

        return view('App\Modules\Strategie\Views\ParticulariteView', $data);
    }
...
public function getList(): mixed
{
    ...
    // TABLE BLOGS
    // =========================
    $query_principal = "SELECT * FROM particularite WHERE 1";
    ...
    if (isset($_POST['order'][0]['column']) && isset($_POST['order'][0]['dir'])) {
        ...
        $order_by = isset($order_column[$colIndex])
            ...
            : ' ORDER BY ID_PARTICULARITE DESC';

    } else {
        $order_by = ' ORDER BY ID_PARTICULARITE DESC';
    }
   ...
   foreach ($fetch_data as $row) {
    ...
    $img = !empty($row->IMAGE)
        ? '<img src="' . base_url('uploads/particularite/' . $row->IMAGE) . '" width="60">'
        : '';
   ...
   $fullDescription = iconv('UTF-8', 'UTF-8//IGNORE', $row->DESCRIPTION);
    ...
    <div class="btn-group">
        ...
        <div class="dropdown-menu">
            <a class="dropdown-item" onclick="editObjectif(' . $row->ID_PARTICULARITE . ')">
                <i class="fa fa-edit"></i> Modifier
            </a>
            <a class="dropdown-item" onclick="deleteObjectif(' . $row->ID_PARTICULARITE . ')">
...
public function save()
{
   ...
   $rules = [
    ...
    $id = $this->request->getPost('ID_PARTICULARITE');
    ...
    try {
        ...
        if ($img && $img->isValid() && !$img->hasMoved()) {
            ...
            $img->move(FCPATH . 'uploads/particularite', $fileName);
        }
    ...
    'IS_ACTIF' => $this->request->getPost('IS_ACTIF')
        ...
        $builder = $db->table('particularite');

        if (!empty($id)) {

            $old = $builder->where('ID_PARTICULARITE', $id)->get()->getRow();

            if ($old && $fileName && !empty($old->IMAGE)) {
                $path = FCPATH.'uploads/particularite/'.$old->IMAGE;
                if (file_exists($path)) unlink($path);
            }

            $builder->where('ID_PARTICULARITE', $id)->update($data);
...
public function getOne($id)
{
    ...
    $data = $db->table('particularite')
        ->where('ID_PARTICULARITE', $id)
        ->get()
        ->getRow();
...
public function delete()
{
    ...
    $db->table('particularite')->where('ID_PARTICULARITE', $id)->delete();
...
{
    return $this->db->table('particularite')
        ->countAllResults();
}
